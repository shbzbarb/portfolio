...
const ALL_WORKS = Object.entries(modules)
  .sort(([a], [b]) => a.localeCompare(b))
  .map(([, mod], i) => ({ ...mod, _num: String(i + 1).padStart(2, '0') }))

export default function Works() {
  const [page, setPage] = useState(0)
  const totalPages = Math.ceil(ALL_WORKS.length / PER_PAGE)
  const hasCarousel = ALL_WORKS.length > PER_PAGE
  const visible = ALL_WORKS.slice(page * PER_PAGE, (page + 1) * PER_PAGE)
  ...
  return (
    <section className="s" id="works">
      <div className="secH">
        ...
        <span className="rule" />
        <span className="meta">ls ./works/</span>
      </div>
        ...
        {visible.map((w) => (
          <div className="w" key={w._num}>
            ...
            <div className="stat-row">
              <span className="stat">{w.stat}</span>
              {w.github && (
